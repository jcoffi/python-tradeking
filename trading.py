...
# Read the authentication keys for TradeKing from yaml.
...
class Trading:
    ...
    def fixml_short_now(self, ticker, quantity, limit):
        ...
        def fixml_sell_trailingstop(self, ticker, quantity, limit):
            ...
            order.set("Side", "2")  # Sell
            order.set("OfstTyp", "1") # 0 for hard number, 1 for percentage
            order.set("PegPxTyp", "1") # 1 means last price
            order.set("OfstVal", "-5")
